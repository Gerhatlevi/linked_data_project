...
path = "C:\\Users\\leven\\Erasmus\\3_quartile\\LDSW\\Project\\linked_data_project\\data_files\\used\\"

def clean_box_office(input_path, output_path):
    df = pd.read_csv(input_path, low_memory=False)
    df = df.drop(columns=['estimated', 'newthisday', 'yd', 'lw', 'daily', 'avg', 'days', 'release'], errors='ignore')
...
def clean_most_popular(input_path, output_path):
    df = pd.read_csv(input_path, low_memory=False)
    df = df.drop(columns=['hours_viewed_first_91_days', 'runtime'], errors='ignore')
    df.to_csv(output_path, index=False, encoding='utf-8')
