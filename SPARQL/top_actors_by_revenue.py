...
SELECT ?personName (SUM(?maxRevenue) AS ?personRevenue)
WHERE {
    {
    SELECT ?movie (MAX(?revenue) AS ?maxRevenue)
    WHERE {
...
for result in results["results"]["bindings"]:
    rows.append({
        ...
        "Total revenue": result["personRevenue"]["value"]
...
table = pd.plotting.table(ax, df, loc="center", cellLoc="center", colWidths=[0.6, 0.3])
